<template>
    ...
    <div class="mx-4 mt-3 pb-7">
        <form @submit.prevent="submit" method="POST">
            ...
            <div class="relative">
                <input type="name" v-model="designation.name" id="name"
    ...
    <div class=" bg-slate-200 mx-4 rounded-lg shadow-sm mb-10">
        <table class=" w-full overflow-scroll">
            ...
            <tbody>
                <tr class="odd:bg-white even:bg-slate-50" v-for="designation in designationState.designations"
                    v-bind:key="designation._id">
                    <td>{{ designation.name }}</td>
                    <td class=" screen-md:hidden">{{ designation.addedBy.name }}</td>
                    <td class=" screen-md:hidden">{{ formatDate(designation.createdAt) }}</td>
                    <td class=" screen-md:hidden">{{ designation.updatedBy.name }}</td>
    ...
    <div id="popup-modal" v-if="popup"
        ...
        <div class="relative bg-white rounded-lg shadow self-center px-20  max-w-md m-auto">
            ...
            <form class=" grid py-20" @submit.prevent="update" method="POST">
                <div class="">
                    <input id="name" name="name" type="name" v-model="updateDesignation.name" autocomplete="name" required=""
...
export default {
    ...
    methods: {
        submit: async function () {
            this.$store.dispatch("createDesignation", { name: this.designation.name, addedBy: this.userState.user._id, updatedBy: this.userState.user._id });
        },
        update: async function () {
            this.$store.dispatch("updateDesignation", { _id: this.updateDesignation._id, name: this.updateDesignation.name, updatedBy: this.userState.user._id });
            this.closePopup();
        },
        formatDate(value) {
            if (value) {
        ...
        openPopup(id, name) {
            this.updateDesignation.name = name;
            this.updateDesignation._id = id;
